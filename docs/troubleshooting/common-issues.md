...
# Check events
kubectl get events -n dclaw-fleet --sort-by='.lastTimestamp'

# Verify database connection
kubectl exec -n dclaw-fleet deployment/dclaw-fleet-backend --   python -c "import asyncio; from sqlalchemy import text; ..."
```
...
2. Check the connection string secret:
   ```bash
   kubectl get secret dclaw-fleet-db-credentials -n dclaw-fleet
   ```
