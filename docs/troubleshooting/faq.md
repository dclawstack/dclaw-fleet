...
### How do I update DClaw Fleet?

Update the `version` field in the DClawApp CRD. The operator will perform a rolling update.

```bash
kubectl patch dclawapp fleet --type merge -p '{"spec":{"version":"0.2.0"}}'
```
...
### Can I run DClaw Fleet without Kubernetes?

Yes, for local development:

```bash
cd dclaw-fleet/frontend && npm run dev
cd dclaw-fleet/backend && uvicorn dclaw_fleet.main:app --reload
```
